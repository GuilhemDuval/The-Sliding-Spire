...
func north():
	$NorthHallway.visible = true
	$NorthWall.queue_free()

func south():
	$SouthHallway.visible = true
	$SouthWall.queue_free()

func west():
	$WestHallway.visible = true
	$WestWall.queue_free()

func east():
	$EastHallway.visible = true
	$EastWall.queue_free()
...
func spawn_node(node_scene : PackedScene, min_ins : int = 0, max_ins : int = 0) -> void:
	...
	for i in range(num):
		...
		var pos : Vector2 = Vector2(48 * randi_range(2, inside_width - 1) - 24 + 144, 48 * randi_range(2, inside_height - 1) - 24 + 144)
		while pos in used_position:
			pos = Vector2(48 * randi_range(2, inside_width - 1) - 24 + 144, 48 * randi_range(2, inside_height - 1) - 24 + 144)
		node_obj.position = pos
		used_position.append(pos)
		add_child(node_obj)
	
	if node_scene == enemy_node:
		get_tree().get_first_node_in_group("EnemyManager").check_enemies()
